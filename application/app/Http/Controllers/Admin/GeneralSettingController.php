...
class GeneralSettingController extends Controller
{
    ...
    public function customCssSubmit(Request $request)
    {
       ...
       public function socialiteCredentials()
       {
           ...
           $general = GeneralSetting::first();
           return view('admin.setting.social_credential', compact('pageTitle','general'));
       }
    ...
    //    agency socialite
    public function socialiteAgencyCredentials()
    {
        $pageTitle = 'Social Login Credentials';
        $general = GeneralSetting::first();
        return view('admin.setting.agency_social_credential', compact('pageTitle','general'));
    }

    public function updateAgencySocialiteCredentialStatus($key)
    {
        $general = GeneralSetting::first();
        $credentials = $general->agency_socialite_credentials;
        try {
            $credentials->$key->status = $credentials->$key->status == 1 ? 0 : 1;
        } catch (\Throwable $th) {
            abort(404);
        ...
        $general->agency_socialite_credentials = $credentials;
        $general->save();
    ...
    public function updateAgencySocialiteCredential(Request $request, $key)
    {
        $general = GeneralSetting::first();
        $credentials = $general->agency_socialite_credentials;
        try {
