...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $widget['total_tour_bookings'] = TourPackage::count();
        

        // Agency Info
        $widget['total_agencies']             = Agency::count();
        $widget['verified_agencies']          = Agency::where('status', 1)->where('ev',1)->where('sv',1)->count();
        $widget['email_unverified_agencies']  = Agency::emailUnverified()->count();
        $widget['mobile_unverified_agencies'] = Agency::mobileUnverified()->count();
        $widget['tickets_agencies'] = SupportTicket::where('agency_id','!=',0)->count();
        ...
        $agencyLoginsReport = UserLogin::selectRaw("COUNT(*) as login_count, DATE_FORMAT(created_at, '%Y-%m-%d') as login_date")
            ->orderBy('login_date', 'desc')
            ->where('agency_id','!=', 0)
            ->groupByRaw("DATE_FORMAT(created_at, '%Y-%m-%d')")
            ->limit(10)
            ->pluck('login_count', 'login_date');
        $agencyLogins['labels'] = $agencyLoginsReport->keys();
        $agencyLogins['values'] = $agencyLoginsReport->values();
            // AgencyLogin Report Graph
